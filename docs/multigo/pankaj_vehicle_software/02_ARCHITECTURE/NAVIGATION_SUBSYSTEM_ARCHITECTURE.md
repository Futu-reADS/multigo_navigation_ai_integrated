...
The **Navigation Subsystem** provides autonomous navigation capabilities for 500m-1km routes without GPS. This architecture **follows proven ParcelPal patterns** (0% code reuse, 70% architectural approach), adapted for swerve drive and wheelchair transport requirements.
...
### 3.2 Localization Node (NDT - Standard ROS 2)

**Node Name:** `ndt_localization`
**Package:** Custom implementation using `ndt_omp` or `pcl::NormalDistributionsTransform` (NOT Autoware)
**Language:** C++
**Note:** Implements ParcelPal's proven 2-phase navigation approach with new code
