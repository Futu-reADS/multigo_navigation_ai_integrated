...
**ドッキングサブシステム**は、ArUcoマーカーを使用したビジュアルサーボイングによる車椅子ベースへの精密取り付けを可能にします。これは車椅子輸送ユースケースのために特別に設計された**新しいサブシステム**（ParcelPalにはない）です。

**主な機能:**
- 二相ドッキング（粗ナビゲーション + 精密ビジュアルサーボイング）
- ArUcoマーカーベース精密位置決め（±2-5mm屋外）
- 信頼性のためのデュアルカメラフュージョン
- 状態機械駆動ドッキングシーケンス
- 安全監視と障害復旧
- 屋外動作能力（小雨、照明変化）

**ドッキング目標:** ±2-5mm位置精度、屋外で>95%成功率
...
**主要コンポーネント:**
1. **ドッキングコーディネーター**: ドッキングシーケンスを管理する状態機械
2. **ArUco検出器**: デュアルカメラからのマーカー検出とポーズ推定
3. **ビジュアルサーボイングコントローラー**: PID制御による位置ベースビジュアルサーボイング（PBVS）
4. **Swerveドライブコントローラー**: 精密動作コマンドを実行
...
**購読トピック:**
- `/aruco_poses` (geometry_msgs/PoseArray): 検出されたArUcoマーカーポーズ
- `/odom` (nav_msgs/Odometry): 現在のロボットオドメトリ
- `/camera_1/image_raw` (sensor_msgs/Image): フロントカメラ画像
- `/camera_2/image_raw` (sensor_msgs/Image): フロント代替カメラ画像

**パブリッシュトピック:**
- `/docking/state` (std_msgs/String): 現在のドッキング状態
- `/docking/status` (custom_msgs/DockingStatus): 詳細ステータスと診断
- `/cmd_vel` (geometry_msgs/Twist): ビジュアルサーボイング中の速度コマンド

**アクションサーバー:**
- `/dock` (custom_msgs/action/Dock): ドッキングシーケンスを実行
- `/undock` (custom_msgs/action/Undock): アンドッキングシーケンスを実行

**アクションクライアント:**
- `/navigate_to_pose` (nav2_msgs/action/NavigateToPose): 粗接近ナビゲーション
...
servoing_limits:
  max_linear_vel: 0.2           # サーボイング中m/s
  max_angular_vel: 0.3          # サーボイング中rad/s
...
**パブリッシュトピック:**
- `/aruco_poses` (geometry_msgs/PoseArray): ロボットフレームでの検出マーカーポーズ
...
**購読トピック:**
- `/aruco_poses` (geometry_msgs/PoseArray): 目標マーカーポーズ
- `/docking/state` (std_msgs/String): ドッキング状態（制御有効/無効）

**パブリッシュトピック:**
- `/cmd_vel` (geometry_msgs/Twist): swerveドライブへの速度コマンド

**パラメータ:**
```yaml
# 制御方法
control_method: "PBVS"          # 位置ベースビジュアルサーボイング
...
```
      ...
      └──────┬──────┘
             ...
             ↓
      ┌─────────────┐
      │  APPROACH   │ ← Nav2ナビゲーションで接近ゾーンへ（±30cm）
      └──────┬──────┘
             │ 接近ゾーン到達
             ↓
      ┌─────────────┐
      ...
      └──────┬──────┘
             ...
             ↓
      ┌─────────────┐
      │  SERVOING   │ ← ビジュアルサーボイングで目標ポーズへ（±5mm）
      └──────┬──────┘
             │ 位置安定（2秒間±5mm）
             ↓
      ┌─────────────┐
      │   DOCKED    │ ← 車椅子にロックコマンド送信
      └──────┬──────┘
             │ /undockアクション呼び出し
             ↓
      ┌─────────────┐
      │ UNDOCKING   │ ← ロック解除、0.5m後退
      └──────┬──────┘
             │ アンドッキング完了
             ↓
      ┌─────────────┐
...
**APPROACH:**
- Nav2 `/navigate_to_pose`アクションを接近ポーズで呼び出し
- 接近ポーズ = ドッキングステーションポーズ + オフセット（前方1.5m）
- 許容範囲: ±30cm（Nav2 xy_goal_tolerance）
- **遷移:** 接近ゾーン到達 → SEARCH

**SEARCH:**
- ArUcoマーカーをスキャンするためゆっくり回転（0.2 rad/s）
- `/aruco_poses`トピックを購読
- **遷移:** マーカー3フレーム連続検出 → SERVOING
- **タイムアウト:** 30秒 → FAILED

**SERVOING:**
- ビジュアルサーボイングコントローラーを有効化
- マーカーポーズ誤差から目標速度を計算
- 10 Hzで`/cmd_vel`をパブリッシュ
- 位置誤差を監視: `error = sqrt((x-x_target)^2 + (y-y_target)^2)`
- **遷移:** 誤差<5mmで2秒間安定 → DOCKED
- **マーカー喪失:** >5s → SEARCH
- **タイムアウト:** 60s → FAILED

**DOCKED:**
- すべての動きを停止（`/cmd_vel` = 0）
- 車椅子にロックコマンド送信（将来: CAN/シリアルインターフェース）
- `/dock`アクションに成功フィードバックをパブリッシュ
- **遷移:** `/undock`アクション呼び出し → UNDOCKING

**UNDOCKING:**
- 車椅子をロック解除（将来: CAN/シリアルインターフェース）
- 0.5m後退（-0.2 m/sで2.5秒のオープンループ動作）
- **遷移:** 動作完了 → IDLE

**FAILED:**
- すべての動きを停止
- `/dock`アクションに失敗フィードバックをパブリッシュ
- 失敗理由をログ（マーカー喪失、タイムアウト、安全違反）
- **遷移:** 自動 → IDLE
...
**アプローチ:**
- APPROACHとSEARCH状態中は3D LiDARを使用（通常のNav2コストマップ）
- SERVOING中は障害物回避を**無効化**（ビジュアルサーボイングパスを信頼）
- 車椅子ベース存在を監視（ドッキング位置での予期される障害物）

**実装:**
```cpp
void updateCostmapConfiguration(DockingState state) {
    if (state == DockingState::SERVOING) {
        // サーボイング中はコストマップインフレーションを無効化
...
**障害モード:**
1. **サーボイング中のマーカー喪失:** SEARCH状態に戻る、マーカー再取得
2. **SERVOINGでのタイムアウト:** 60秒後、FAILEDを宣言、IDLEに戻る
3. **不安定なマーカーポーズ:** フレーム間で>10cmポーズジャンプがある場合、外れ値を拒否
4. **カメラ障害:** 両カメラが故障した場合、ドッキング中止
5. **接近中の衝突:** Nav2が障害物を検出、ドッキング中止
...
### 8.1 単体テスト
- ArUco検出精度（既知マーカー → 期待ポーズ）
- PnPポーズ推定正確性
- デュアルカメラフュージョンアルゴリズム
- ビジュアルサーボイング制御則（誤差 → 速度）
- 状態機械遷移

### 8.2 統合テスト
- 記録されたカメラ画像でのArUco検出器ノード
- シミュレートされたマーカーポーズでのビジュアルサーボイングコントローラー
...
| マーカー検出レート | 10 Hz | タイムスタンプ分析 |
| ビジュアルサーボイング制御レート | 10 Hz | タイムスタンプ分析 |
| ドッキング時間（APPROACH→DOCKED） | <30s | フィールドテスト |
| マーカー喪失復旧時間 | <5s | 障害復旧テスト |
...
**文書ステータス:** ドラフト
**実装ステータス:** 未開始
**必要な承認:** ドッキングエンジニア、ビジョンエンジニア、ナビゲーションリード、安全エンジニア
