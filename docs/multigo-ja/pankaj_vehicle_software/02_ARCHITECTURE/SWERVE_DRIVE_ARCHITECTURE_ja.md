...
**Swerveドライブサブシステム**は、車椅子輸送ロボットの全方向移動制御を提供します。ParcelPalの差動駆動とは異なり、このサブシステムはホロノミック運動（vx、vy、ωの独立制御）を可能にし、狭いスペースでの精密な操縦を実現します。

**主な機能:**
- 4輪独立swerveモジュール（車輪ごとの駆動+ステアリング）
- 全方向移動（回転なしであらゆる方向に移動）
- Nav2コントローラー統合
- リアルタイム逆運動学
- 安全準拠速度制限
...
**コンポーネント:**
1. **Swerveドライブコントローラーノード** (C++): nav2_core::Controllerインターフェース実装のメインコントローラー
2. **モジュールドライバーノード** (4×): 車輪ごとの制御ノード（駆動モーター + ステアリングモーター）
3. **モーターコントローラー** (8×): ハードウェアコントローラー（4駆動 + 4ステアリング）
4. **エンコーダー** (8×): 位置フィードバック（4車輪速度 + 4ステアリング角度）

---

## 3. ROS 2ノードアーキテクチャ

### 3.1 Swerveドライブコントローラーノード
...
**パブリッシュトピック:**
- `/wheel_commands` (custom_msgs/SwerveModuleCommand): 車輪ごとの駆動/ステアリングコマンド
...
**課題:** ステアリング遷移中のモジュール「競合」を回避

**解決策:** すべてのモジュールを目標角度に到達させてから駆動力を適用するよう調整
...
void updateModules(SwerveCommand cmd) {
    ...
    // 2. >90°回転が必要なモジュールがあるかチェック
    //    ある場合、角度を反転して駆動方向を逆転（最適化）
    for (int i = 0; i < 4; i++) {
    ...
    // 3. すべてのモジュールが目標角度に到達するまで待機（許容範囲内）
...
**主要メソッド:**
- `computeVelocityCommands()`: パスに沿うボディ速度を計算
...
**制限:**
- 最大線速度: 1.5 m/s（乗客なし）、1.0 m/s（乗客あり）
- 最大角速度: 2.0 rad/s
- 最大加速度: 0.5 m/s²
...
void emergencyStop() {
    ...
    for (int i = 0; i < 4; i++) {
        sendDriveCommand(i, 0.0);
        // 現在のステアリング角度を保持（急な動きを避けるためリセットしない）
    }
...
- 全方向移動（左右にストレイフ、斜め）
- 精密位置決め（±10cm精度）
- スムーズな遷移（ステアリング変更中のジャーキー動作なし）
- 負荷下パフォーマンス（100kgペイロード）
...
**文書ステータス:** ドラフト
**実装ステータス:** 未開始
**必要な承認:** ナビゲーションリード、Swerveドライブエンジニア、安全エンジニア
